...
//get the time reletive to start time
static uint64_t get_time() {
  uint64_t now = get_time_internal();
  return now - boot_time;
}

//pinmap is used to describe pin on the nvboard
...
uint64_t input_map[NR_INPUT_PINS] = {0}; // 24 in total
uint64_t output_map[NR_OUTPUT_PINS] = {0}; // 114 in total
std::string nvboard_home;

int read_event();

//firstly, p->signal is the interface of verilator;
//we get the parameter at nvboard,then we transfer it to verilator;
//In more detail, there is a input map and a output map in the nvboard,
//in the map, nvboard get the 0 or 1 from user and put it in the respective map.
//finally, signal is assigned as the integer.
...
void nvboard_init(int vga_clk_cycle) {
    printf("NVBoard v0.2\n");
    printf("OUTPUT_PIN_NUM:%d\n",NR_OUTPUT_PINS);
    printf("INPUT_PIN_NUM:%d\n",NR_INPUT_PINS);
