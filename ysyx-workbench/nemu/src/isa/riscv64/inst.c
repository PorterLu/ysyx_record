...
void delete_ftrace();
void add_etrace(uint32_t NO, paddr_t pc, bool is_call);
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  */           
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(dest) = read_csr(src2); set_csr(src2, src1));
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, s->dnpc = isa_raise_intr(2, cpu.pc, cpu.gpr[17]););
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(dest) = read_csr(src2); set_csr(src2, read_csr(src2) | src1));
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , R, s->dnpc = read_csr(MEPC));
